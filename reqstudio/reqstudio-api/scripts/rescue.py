...
async def check_health():
    ...
    try:
        ...
        async with engine.connect() as conn:
            ...
            # 2. Check Tables
            table_queries = {
                "users": text("SELECT 1 FROM users LIMIT 1"),
                "tenants": text("SELECT 1 FROM tenants LIMIT 1"),
                "projects": text("SELECT 1 FROM projects LIMIT 1"),
                "sessions": text("SELECT 1 FROM sessions LIMIT 1"),
                "artifacts": text("SELECT 1 FROM artifacts LIMIT 1"),
            }
