...
@pytest.mark.asyncio
async def test_export_respects_tenant_isolation(client: AsyncClient, tenant_a_token, tenant_b_token):
    artifact_id = await _create_base_artifact(client, tenant_a_token)
...
@pytest.mark.asyncio
async def test_export_markdown_nfr3_baseline_50_pages_equivalent(client: AsyncClient, tenant_a_token):
    p_res = await client.post(
        "/api/v1/projects",
    ...
    sections = []
    paragraph = ("Requisito detalhado com critérios de negócio e regras operacionais. " * 30).strip()
    for i in range(1, 51):
        sections.append(
            {
                "id": f"s{i}",
