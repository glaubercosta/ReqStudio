/**
 * ProjectsPage — dashboard de projetos (Story 3.2).
 *
 * Grid responsivo: 3-4 col desktop → 2 tablet → lista mobile.
 * Estados: skeleton (loading) → empty state → grid de cards.
 */
        ...
        <div className="flex items-start justify-between gap-[var(--space-4)] mb-[var(--space-5)]">
          <div>
            ...
            <p className="text-body-sm text-muted-foreground mt-[var(--space-1)]">
              {isLoading
                ? 'Carregando...'
                : `${data?.total ?? 0} projeto${(data?.total ?? 0) !== 1 ? 's' : ''}`}
            </p>
          </div>
          <Button
            id="btn-new-project"
